			...
			<section class="section">
				<div class="ly_inner md">
					...
					<article class="search_box type2 mb0">
						<form action="photoList.do" class="search_box_form grid" id="searchForm">
					...
					<article class="mt50">
						...
						<div class="board_header">
							<div class="bbs_sort_list">
								<a href="photoList.do?searchKeyword=${searchKeyword}&sdate=${sdate}&edate=${edate}&otype=num" class="sort <c:if test="${otype =='num'}">active</c:if>">최신순</a>
								<a href="photoList.do?searchKeyword=${searchKeyword}&sdate=${sdate}&edate=${edate}&otype=visited" class="sort <c:if test="${otype =='visited'}">active</c:if>">조회순</a>
							</div>
						</div>
						<!-- //board header -->
						...
						<c:forEach items="${photoList}" var="photoList">	
							<li class="item">
								<a href="photoDetail.do?num=${photoList.num}&listpage=${currentPage}&searchKeyword=${searchKeyword}&sdate=${sdate}&edate=${edate}&otype=${otype}" class="bbs_box">
									<div class="el_thumb rds el_img media">
										<c:if test="${photoList.img1 != null && photoList.img1 != ''}">
											<img src="<c:url value='/resources/common/images/upload/gallery/${photoList.img1 }'/>" alt="" >
										</c:if>
										<c:if test="${photoList.img1 == null || photoList.img1 == ''}">
									...
									<p class="bbs_info_wrap">
										<span class="bbs_info date">${photoList.formatted_date}</span>
					...
					<a href="photoList.do?page=1&searchKeyword=${searchKeyword}&sdate=${sdate}&edate=${edate}&otype=${otype}" class="page_link ico first"><span class="blind">처음</span></a> <!-- 비활성화시 disabled 추가 -->
					</c:if>
					
					<!-- 이전 블럭으로 이동 -->
					<c:if test="${startPage2 gt 1 }">
                       	<a href="photoList.do?page=${startPage2-1}&searchKeyword=${searchKeyword}&sdate=${sdate}&edate=${edate}&otype=${otype}" class="page_link ico prev"><span class="blind">이전</span></a> <!-- 비활성화시 disabled 추가 -->
                    </c:if>
                    ...
                    <c:forEach var="p" begin="${startPage2}" end="${endPage2}" step="1">
                      ...
                      <c:if test="${p ne currentPage }">
                      	<c:url var="photoList" value="photoList.do?searchKeyword=${searchKeyword}&sdate=${sdate}&edate=${edate}&otype=${otype}">
	 					<c:param name="page" value="${p}" />
	 					</c:url>
	 					<a href="${photoList}" class="page_link">${p}</a>
                      </c:if>
                    </c:forEach>
                    
                    <!-- 다음 블럭으로 이동 -->
                    <c:if test="${endPage2 ne maxPage2 && maxPage2 > 1}">
					<a href="photoList.do?page=${endPage+1}&searchKeyword=${searchKeyword}&sdate=${sdate}&edate=${edate}&otype=${otype}" class="page_link ico next"><span class="blind">다음</span></a>
                    </c:if>
                    ...
                    <c:if test="${maxPage2 > 0}">
					<a href="photoList.do?page=${maxPage}&searchKeyword=${searchKeyword}&sdate=${sdate}&edate=${edate}&otype=${otype}" class="page_link ico last"><span class="blind">마지막</span></a>
					</c:if>
				</div>
				<!-- // pagination -->
						<!-- pagination -->
					...
					<c:if test="${startPage gt 1 }">
                       	<a href="photoList.do?page=${startPage-1}&searchKeyword=${searchKeyword}&sdate=${sdate}&edate=${edate}&otype=${otype}" class="page_link ico prev"><span class="blind">이전</span></a> <!-- 비활성화시 disabled 추가 -->
                    </c:if>
