...
<head>
	...
	<meta property="og:title" content="뉴스 : KCC이지스 프로농구단">
	<meta property="og:image" content="https://kccegis.com/resources/common/images/common/kcc_og_thum.jpg">
	<meta property="og:description" content="부산KCC이지스 공식 홈페이지">
	<title>뉴스 : KCC이지스 프로농구단</title>
					...
					<div class="inner">
						<div class="snb_2dth snb_list menu_slider">
							<div class="swiper-wrapper">
								<a href="newsList.do" class="swiper-slide snb_link current"><span>뉴스</span></a>  <!-- 해당페이지에 current 추가 -->
			...
			<section class="section">
				<div class="ly_inner md">
					...
					<article class="search_box type2 mb0">
						<form action="newsList.do" class="search_box_form grid" id="searchForm">
							<div class="row row1">
					...
					<article class="mt50">
						...
						<c:forEach items="${newsList}" var="newsList">
							<li class="item">
								<a href="${newsList.linkurl}" class="bbs_box" target="_blank" rel="noreferrer" aria-label="기사 보러 가기(새창열림)">
									<div class="el_thumb rds el_img media outlink">
									...
									<p class="bbs_tit txt_line2">${newsList.subject}</p>
									<p class="bbs_info_wrap">
										<span class="bbs_info date">${newsList.formatted_date}</span>
										<span class="bbs_info press">${newsList.etc1}</span>
									</p>
								</a>
							</li>
						</c:forEach>
				...
				<div class="pagination xm_hide">
					...
					<a href="newsList.do?page=1&searchKeyword=${searchKeyword}&season=${season}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}" class="page_link ico first"><span class="blind">처음</span></a> <!-- 비활성화시 disabled 추가 -->
					</c:if>
					
					<!-- 이전 블럭으로 이동 -->
					<c:if test="${startPage2 gt 1 }">
                       	<a href="newsList.do?page=${startPage2-1}&searchKeyword=${searchKeyword}&season=${season}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}" class="page_link ico prev"><span class="blind">이전</span></a> <!-- 비활성화시 disabled 추가 -->
                    </c:if>
                    ...
                    <c:forEach var="p" begin="${startPage2}" end="${endPage2}" step="1">
                      ...
                      <c:if test="${p ne currentPage }">
                      	<c:url var="newsList" value="newsList.do?&searchKeyword=${searchKeyword}&season=${season}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}">
	 					<c:param name="page" value="${p}" />
	 					</c:url>
	 					<a href="${newsList}" class="page_link">${p}</a>
                      </c:if>
                    </c:forEach>
                    
                    <!-- 다음 블럭으로 이동 -->
                    <c:if test="${endPage2 ne maxPage2 && maxPage2 > 1}">
					<a href="newsList.do?page=${endPage+1}&searchKeyword=${searchKeyword}&season=${season}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}" class="page_link ico next"><span class="blind">다음</span></a>
                    </c:if>
                    ...
                    <c:if test="${maxPage2 > 0}">
					<a href="newsList.do?page=${maxPage2}&searchKeyword=${searchKeyword}&season=${season}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}" class="page_link ico last"><span class="blind">마지막</span></a>
					</c:if>
				</div>
				<!-- // pagination -->
						<!-- pagination -->
				<div class="pagination xm_show">
					...
					<c:if test="${startPage gt 1 }">
                       	<a href="newsList.do?page=${startPage-1}&searchKeyword=${searchKeyword}&season=${season}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}" class="page_link ico prev"><span class="blind">이전</span></a> <!-- 비활성화시 disabled 추가 -->
                    </c:if>
                    ...
                    <c:if test="${maxPage > 0}">
					<a href="newsList.do?page=${maxPage}&searchKeyword=${searchKeyword}&season=${season}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}" class="page_link ico last"><span class="blind">마지막</span></a>
					</c:if>
				</div>
				<!-- // pagination -->
						</c:if>
